...
export const DirectoryButton = ({ parentPath, name, type, icon }) => {
    ...
    return Widget.Button({
        ...
        onClicked: () => {
            App.closeWindow('overview');
            execAsync(['zsh', '-c', `xdg-open '${parentPath}/${name}'`, `&`]).catch(print);
        },
        child: Widget.Box({
            children: [
                Widget.Box({
...
export const ExecuteCommandButton = ({ command, terminal = true }) => searchItem({
...
export const SearchButton = ({ text = '' }) => searchItem({
    ...
    onActivate: () => {
        ...
        execAsync(['zsh', '-c', `xdg-open '${search}' &`]).catch(print);
    },
});
